# PowerOfficeGoV2.Model.SupplierLedgerEntryDto
...
**DueDate** | **DateOnly** | The due date of the entry. Relevant only for voucher types IncomingInvoice and IncomingCreditNote. The date represent when the amount is due to be paid to the supplier. | [optional] 
...
**InvoiceNo** | **string** | The invoice number, if the entry is an entry with the voucher type IncomingInvoice or IncomingCreditNote.  Be aware that the invoiceNo is a string, so it&#39;s not necessarily a number. | [optional] [readonly] 
...
**LastChangedDateTimeOffset** | **DateTime** | The timestamp of the last change of this entry.  The accounting entry in itself cannot change, but the match state of the sub-ledger entry might change.  Using last changed filtering is one way of retrieving new matched items and as such the payment status of invoices. | [optional] [readonly] 
