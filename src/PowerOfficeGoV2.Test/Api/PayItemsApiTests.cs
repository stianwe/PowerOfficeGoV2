...
{
    /// <summary>
    ///  Class for testing PayItemsApi
    /// </summary>
    public sealed class PayItemsApiTests : ApiTestsBase
    {
        private readonly IPayItemsApi _instance;

        public PayItemsApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IPayItemsApi>();
        }
        ...
        public async Task GetPayItemByIdAsyncTest()
        {
            Guid id = default!;
            var response = await _instance.GetPayItemByIdAsync(id);
            var model = response.Ok();
            Assert.IsType<PayItemDto>(model);
        }
        ...
        public async Task PayItemsGetAsyncTest()
        {
            ...
            var model = response.Ok();
            Assert.IsType<List<PayItemDto>>(model);
        }
    }
}
